...
class Blockchain:
    ...
    def create_genesis_block(self):
        ...
        previous_hash.update(
            ''.encode('utf-8'))  # the previous hash is simply a placeholder created by hashing an empty string
    ...
    def proof_of_work(self, block, previous=-1):
        ...
        '''
        return int(block.hashid, 16) < 2 ** (256 - self.difficulty) and block.previous_hash == self.chain[
    ...
    def validate_another_chain(self, chain):
        ...
        while block_index < len(
            ...
            print("val1 ",int(block.hashid, 16) < 2 ** (256 - self.difficulty))
            print("val2 ",block.previous_hash == chain[
                block_index - 1].hashid)
            print("val3 ", int(block.hashid, 16) < 2 ** (256 - self.difficulty) and block.previous_hash == chain[
                block_index - 1].hashid)
            result = int(block.hashid, 16) < 2 ** (256 - self.difficulty) and block.previous_hash == chain[
