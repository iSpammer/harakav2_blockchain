...
class Blockchain:
    ...
    def create_genesis_block(self):
        ...
        # print("prev hash 1", previous_hash)
        # previous_hash.update(''.encode('utf-8')) # the previous hash is simply a placeholder created by hashing an empty string
        previous_hash = haraka512256(harakav2.pad_message(('QRNG'.encode())))
        print("prev hash 2", previous_hash)
        previous_hash = bytearray(previous_hash).hex()
        print("prev hash 3", previous_hash)
    ...
    def proof_of_work(self, block, previous=-1):
        ...
        '''
        print ("1xd ", int(block.hashid, 16) < 2 ** (450 - self.difficulty))
        print("2xd ", block.previous_hash == self.chain[
            previous].hashid)
        print("3xd ",int(block.hashid, 16) < 2 ** (450 - self.difficulty) and block.previous_hash == self.chain[
            previous].hashid)
        return int(block.hashid, 16) < 2 ** (450 - self.difficulty) and block.previous_hash == self.chain[
            previous].hashid  # two conditions
    ...
    def add_to_transactions(self, country, state_code, state, org, org_unit, cname, email, pub_key_s_h):
        '''
        ...
        transaction_dictionary['country'] = country
        transaction_dictionary['state_code'] = state_code
        transaction_dictionary['state'] = state
        transaction_dictionary['org'] = org
        transaction_dictionary['org_unit'] = org_unit
        transaction_dictionary['cname'] = cname
        transaction_dictionary['email'] = email
        transaction_dictionary['pub_key_s_h'] = pub_key_s_h
    ...
    def mine_transactions(self):
        ...
        if len(self.transactions) > 0:  # checks for pending transactions
            ...
            block = Block(len(self.chain) + 1, data,
                          self.chain[-1].hashid)  # block number and previous hash obtained from chain list
    ...
    def validate_chain(self):
        ...
        while block_index < len(
                self.chain):  # goes through each block in the chain starting with the block after the genesis block
    ...
    def validate_another_chain(self, chain):
        ...
        block_index = 1
        while block_index < len(
                chain):  # goes through each block in the chain starting with the block after the genesis block
            block = chain[block_index]
            result = int(block.hashid, 16) < 2 ** (450 - self.difficulty) and block.previous_hash == chain[
                block_index - 1].hashid  # checks the two requirements
            if result == False:
                return False
            block_index += 1
        return True  # entire chain is valid
    ...
    def find_longest_chain(self):
        ...
        for node in other_nodes:
            chain_details = requests.get(
                f'http://{node}/chain_details')  # obtain the chain data from the other nodes server
            print(chain_details.status_code)
            if chain_details.status_code == 200:
                chain = jsonpickle.decode(
                    chain_details.json()['Chain'])  # have to deserialize since we pass in a complex object to json
